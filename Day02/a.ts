...
for (let i = 0; i < lines.length; i++) {
  ...
  if (isGamePossible(gameInfo)) sum += currGameId;
}
console.log(sum);
...
function isGamePossible(gameInfo: GameInfo) {
  if (gameInfo.length === 0) return false;

  for (const round of gameInfo) {
    if (round.blue && round.blue > MAXMIMUMINROUND.blue) return false;
    if (round.green && round.green > MAXMIMUMINROUND.green) return false;
    if (round.red && round.red > MAXMIMUMINROUND.red) return false;
  }
  return true;
}
